...
function customCursor() {
  ...
  if (wrapper && !isMobile.any()) {
    let mouseActions2 = function(e) {
      const isNowInsideWrapper = wrapper.contains(e.target) || e.target === wrapper;
      if (!isInsideWrapper && isNowInsideWrapper) {
        cursor.style.removeProperty("opacity");
        isInsideWrapper = true;
      } else if (isInsideWrapper && !isNowInsideWrapper) {
      ...
      if (e.type === "mouseout") {
        cursor.style.opacity = 0;
        isInsideWrapper = false;
      } else if (e.type === "mousemove" && isInsideWrapper) {
      ...
      } else if (e.type === "mousedown" && isInsideWrapper) {
        cursor.classList.add("--active");
      } else if (e.type === "mouseup" && isInsideWrapper) {
...
document.getElementById("date").min = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
document.fonts.ready.then(() => {
  const blockTitles = gsapWithCSS.utils.toArray(".blocktitle__title");
  blockTitles.forEach((blockTitle) => {
    const split = new SplitText(blockTitle, { type: "words,chars" });
    gsapWithCSS.from(
      split.chars,
      {
        opacity: 0,
...
document.fonts.ready.then(() => {
  ...
  const split = new SplitText(mainTitle, { type: "words,chars" });
  gsapWithCSS.fromTo(
    split.chars,
    { y: 50, opacity: 0 },
    {
...
document.fonts.ready.then(() => {
  const menuCards = gsapWithCSS.utils.toArray(".foods__card");
